...
def wallFollower(m):
    global direction
    direction={'forward':'N','left':'W','back':'S','right':'E'} #standard directions
    currCell=(m.rows,m.cols)#last cell
    path='' #stores the path taken to arrive at the solution

    while True:
        
        if currCell==(1,1): #reached the solution
            break
    ...
    #removing loops and dead ends from the path taken
    path2=path 
...
if __name__=='__main__':
    ...
    # a=agent(myMaze,shape='arrow',footprints=True)  justs shows the path the arrow is following
