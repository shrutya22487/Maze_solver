...
def count_dead_ends(grid,algo):
    count = 0
    for x in range (0, width):
        for y in range(0, height):
            if (grid[x][y].N + grid[x][y].E + grid[x][y].S + grid[x][y].W)  == 1:
                count+=1
    ...
    if algo == 1:
        g.write(str(count) + "\n")
    print("Number of dead end cells=", count)
...
def left_wall_follower():
    ...
    while not (x == 0 and y == 0):
        ...
        y * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2, x * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2,
...
def prims_algorithm():
    ...
    while frontier:  # selecting random frotier
        ...
        pygame.display.update()
        clock.tick(FPS)
    count_dead_ends(grid,1)


def dikshtra():
    def draw_path(path):
        for i in path:
            pygame.draw.rect(screen, GREEN, (i[1] * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2,
                                             i[0] * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2, SMALL_BLOCK_SIZE,
                                             SMALL_BLOCK_SIZE))
        ...
        for i in "NEWS":
            ...
            if childCell != 0:
                pygame.draw.rect(screen, GRAY, (childCell[1] * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2,
                                                childCell[0] * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2,
                                                SMALL_BLOCK_SIZE,
                                                SMALL_BLOCK_SIZE))
...
def dfs1(option):
    ...
    while stack:
        ...
        if (x, y) == (width - 1, height - 1):  # Reached the exit
            ...
            print("len of sol path: ",  len(visited)+ len_of_sol_path)
            if (option==0):
                f.write(str(len(visited)+ len_of_sol_path)+ "\n")
            if (option==1):
                g.write(str(len(visited)+ len_of_sol_path)+ "\n")
            print("no. of cell visited dfs ", len(visited))
            h.write(str(len(visited))+ "\n")
...
def dead_end_filler1():
    ...
    while currcell != (width - 1, height - 1):
        pygame.draw.rect(screen, GREEN, (currcell[1] * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2,
                                         currcell[0] * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2,
                                         SMALL_BLOCK_SIZE, SMALL_BLOCK_SIZE))
        pygame.display.update()
        visited.append(currcell)
...
def dead_end_filler():
    ...
    pygame.time.delay(3000)
    print("Number of dead-end cells:", dead_end_count)



def draw_solution_path1(cell, visited):
    for x, y in visited:
        pygame.draw.rect(screen, PINK, (y * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2,
                                        x * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2, SMALL_BLOCK_SIZE,
                                        SMALL_BLOCK_SIZE))
...
def draw_solution_path(cell, visited):
    ...
    pygame.draw.rect(screen, PINK, (cell[1] * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2,
                                    cell[0] * CELL_SIZE + (CELL_SIZE - SMALL_BLOCK_SIZE) // 2, SMALL_BLOCK_SIZE,
                                    SMALL_BLOCK_SIZE))
    print("Number of cells in the solution path:", solution_path_count)
